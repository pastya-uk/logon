...
import UaaLogon from '../../src/index';
...
export const displayLock = (name, opts = {}, done = () => {}, show_ops = {}) => {
  ...
  const lock = new UaaLogon("cid", "domain", opts);
...
const q = (lock, query, all = false) => {
  query = `#uaa-logon-container-${lock.id} ${query}`;
...
const qView = (lock, query, all = false) => {
  ...
  const view = q(lock, ".uaa-logon-view-content");
  const method = all ? "querySelectorAll" : "querySelector";
  return view ? view[method](query) : null;
};

export function qInput(lock, name, ensure = false) {
  const input = qView(lock, `.uaa-logon-input-${name} input`);
  if (ensure && !input) {
...
const isTabCurrent = (lock, regexp) => {
  ...
  // mechanism.
  const currentTabs = qView(lock, ".uaa-logon-tabs-current", true);
  return currentTabs.length === 1
    && currentTabs[0].innerText.match(regexp);
};

export const hasAlternativeLink = hasViewFn(".uaa-logon-alternative-link");
export const hasBackButton = hasFn(".uaa-logon-back-button");
export const hasEmailInput = hasInputFn("email");
export const hasLoginSignUpTabs = hasViewFn(".uaa-logon-tabs");
export const hasNoQuickAuthButton = lock => {
  return !qView(lock, ".uaa-logon-socia-button");
};
...
export const hasOneSocialButton = hasOneViewFn(".uaa-logon-social-button");
export const hasOneSocialBigButton = hasOneViewFn(".uaa-logon-social-button.uaa-logon-social-big-button");
export const hasPasswordInput = hasInputFn("password");
export const hasTermsCheckbox = hasFn(".uaa-logon-sign-up-terms-agreement label input[type='checkbox']");
export const hasQuickAuthButton = (lock, icon, domain) => {
  // TODO: we should actually check that there's just a single button
  const xs = qView(lock, `.uaa-logon-social-button[data-provider^="${icon}"]`, true);
  return xs.length === 1 && xs[0].innerText.toLowerCase().indexOf(domain) !== -1;
};
export const hasSocialButtons = hasViewFn(".uaa-logon-social-button");
export const hasSSONotice = hasViewFn(".auth0-sso-notice-container");
export const hasSubmitButton = hasFn("button.uaa-logon-submit");
...
export const isSubmitButtonDisabled = hasFn("button.uaa-logon-submit[disabled]");
...
export const clickTermsCheckbox = checkFn(".uaa-logon-sign-up-terms-agreement label input[type='checkbox']");
export const clickSocialConnectionButton = (lock, connection) => clickFn(lock, `.uaa-logon-social-button[data-provider='${connection}']`);
...
export const submitForm = (lock) => {
  const form = q(lock, ".uaa-logon-widget");
...
export const waitUntilInputExists = (lock, name, cb, timeout) =>
  waitUntilExists(lock, `.uaa-logon-input-${name} input`, cb, timeout);
