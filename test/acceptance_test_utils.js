...
import UaaLogonPasswordless from '../src/index';
...
const CURRENT_CRED_PANE_SELECTOR = ".uaa-logon-cred-pane:not(.horizontal-fade-leave):not(.reverse-horizontal-fade-leave)";
...
export function constructLock(cid = "a", domain = "a") {
  return new UaaLogonPasswordless(cid, domain);
}
...
export function isOpened(lock) {
  ...
  return node && CSSCore.hasClass(node, "uaa-logon-opened");
}

export function closeLock(lock) {
  const node = q(lock, ".uaa-logon-close-button");
  if (!node) {
...
export function qInput(lock, name, ensure = false) {
  ...
  const input = q(lock, `${CURRENT_CRED_PANE_SELECTOR} .uaa-logon-input-${name} input`);
  if (ensure && !input) {
...
export function isInputInvalid(lock, name) {
  ...
  return CSSCore.hasClass(node, "uaa-logon-error");
}
...
export function isLoading(lock) {
  ...
  return CSSCore.hasClass(node, "uaa-logon-mode-loading");
}
...
export function isShowingConfirmation(lock) {
  return !!q(lock, ".uaa-logon-confirmation:not(.slide-leave)");
}

function qResendLink(lock, ensure = false) {
  const node = q(lock, ".uaa-logon-resend-link");
  if (ensure && !node) {
...
export function hasLinkBeenResent(lock) {
  const node = q(lock, ".uaa-logon-sent-label")
  if (!node) {
...
export function hasResendingFailed(lock) {
  const node = q(lock, ".uaa-logon-sent-failed-label")
  if (!node) {
...
export function isShowingLocationSelector(lock) {
  return isShowingAuxiliaryPane(lock, ".uaa-logon-select-country");
}
...
export function qLocations(lock) {
  ...
  return q(lock, `.uaa-logon-select-country${AUXILIARY_PANE_SELECTOR_SUFFIX} .uaa-logon-list-code > ul > li`, true);
}
...
export function filterLocations(lock, value) {
  ...
  const input = q(lock, `.uaa-logon-select-country${AUXILIARY_PANE_SELECTOR_SUFFIX} .uaa-logon-search input`);
  if (!input) {
