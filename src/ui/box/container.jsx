...
const BottomBadge = ({link}) => (
  <span className="uaa-logon-badge-bottom">
    <a href={link} target="_blank" className="uaa-logon-badge">
...
const Avatar = ({imageUrl}) => (
  <img src={imageUrl} className="uaa-logon-header-avatar" />
);
...
export default class Container extends React.Component {
  ...
  render() {
    ...
    const overlay = isModal
      ? <div className="uaa-logon-overlay">{badge}</div>
      : null;

    let className = "uaa-logon";

    if (isModal && this.state.isOpen) {
      className += " uaa-logon-opened"
    }

    if (!isModal) {
      className += " uaa-logon-opened-in-frame";
    }

    if (isMobile) {
      className += " uaa-logon-mobile";
    }

    if (isSubmitting) {
      className += " uaa-logon-mode-loading";
    }

    if (auxiliaryPane) {
      className += " uaa-logon-auxiliary";
    }

    if (!submitHandler) {
      className += " uaa-logon-no-submit";
    }

    if (terms) {
      className += " uaa-logon-with-terms";
    }

    if (IPHONE) {
      className += " uaa-logon-iphone";
    }

    // TODO: this no longer makes sense, instead of taking a tabs
    // prop we should take extra class names.
    if (tabs) {
      className += " uaa-logon-with-tabs";
    }

    return (
      <div className={className} ref="container">
        {overlay}
        <div className="uaa-logon-center">
          <form className="uaa-logon-widget" onSubmit={::this.handleSubmit}>
            {avatar && <Avatar imageUrl={avatar} />}
            {closeHandler && <CloseButton onClick={::this.handleClose} />}
            <div className="uaa-logon-widget-container">
              <Chrome
                autofocus={autofocus}
                avatar={avatar}
